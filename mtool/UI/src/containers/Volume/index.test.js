...
describe('<Storage Management />', () => {
  ...
  it('should add a spare disk', async () => {
    ...
    renderComponent();
    const { getByTestId } = wrapper;
    const attachButton = await waitForElement(() => getByTestId('attachdisk-5'));
    fireEvent.click(attachButton);
    fireEvent.click(await waitForElement(() => getByTestId("alertbox-yes")));
    expect(getSpy).toHaveBeenCalledWith('/api/v1.0/add_spare_device/', {
      name: "intel-unvmens-5",
      array: "POSArray"
    }, {
      "headers": {
  ...
  it('should remove a spare disk', async () => {
    ...
    renderComponent();
    const { getByTestId } = wrapper;
    const attachButton = await waitForElement(() => getByTestId('detachdisk-3'));
    fireEvent.click(attachButton);
    fireEvent.click(await waitForElement(() => getByTestId("alertbox-yes")));
    expect(getSpy).toHaveBeenCalledWith('/api/v1.0/remove_spare_device/', {
      name: "intel-unvmens-3",
      array: "POSArray"
    }, {
      "headers": {
  ...
  it('should create a volume', async () => {
    mock.onGet(/api\/v1.0\/get_devices\/*/).reply(200, {
      ...
      .onGet(/api\/v1.0\/get_arrays\/*/).reply(200, [array])
      .onGet(/api\/v1.0\/get_volumes\/*/).reply(200, [])
      .onGet(/api\/v1.0\/available_storage\/*/).reply(200, [{ "arraySize": 2821793513472, "usedSpace": 2821786173440 }])
      .onPost(/api\/v1.0\/delete_array\/*/).reply(200, {})
    ...
    renderComponent();
    const { getByTestId, getByLabelText, getAllByTitle, asFragment, getByText, getAllByText } = wrapper;
  ...
  it('should throw error if creating volume is not possible because of wrong values', async () => {
    ...
    fireEvent.click(createVolButton);
    expect(await waitForElement(() => getByText('Please Enter Volume Size'))).toBeDefined();
    fireEvent.click(await waitForElement(() => getByText('OK')));

    fireEvent.change(volSize, { target: { value: -1 } });
    fireEvent.click(createVolButton);
    expect(await waitForElement(() => getByText('Volume Size cannot be negative'))).toBeDefined();
    fireEvent.click(await waitForElement(() => getByText('OK')));
    ...
    // fireEvent.click(await waitForElement(() => getByText('OK')));
    const volMaxIOPS = await waitForElement(() => getByTestId('create-vol-max-iops'));
    fireEvent.change(volMaxIOPS, { target: { value: 5 } });
    fireEvent.click(createVolButton);
    expect(await waitForElement(() => getByText('Invalid value for Maximum IOPS'))).toBeDefined();
    fireEvent.click(await waitForElement(() => getByText('OK')));
  ...
  it('should try to mount a volume', async () => {
    mock.onGet(/api\/v1.0\/get_devices\/*/).reply(200, {
      ...
      .onPost(/api\/volume\/mount/).reply(200, {
        result: {
          status: {
            code: 1024,
    ...
    const mountBtn = await waitForElement(() => getByTestId('vol-mount-btn-vol2'));
  ...
  it('should unmount a volume', async () => {
    mock.onGet(/api\/v1.0\/get_devices\/*/).reply(200, {
      ...
      .onDelete(/api\/volume\/mount/).reply(200, {
        result: {
          status: {
            code: 0,
            description: "Success"
  ...
  it('should edit a volume', async () => {
    mock.onGet(/api\/v1.0\/get_devices\/*/).reply(200, {
      ...
      .onPut('/api/v1.0/update-volume/').reply(200, {
        result: {
          status: {
            code: 0,
            description: "Success"
    ...
    const volName = await waitForElement(() => getByTestId('list-vol-name-vol2'));
  ...
  it('should show error mesasage if invalid value is given to maxiops', async () => {
    ...
    const saveBtn = await waitForElement(() => getByTestId('vol-edit-save-btn-vol2'));
    fireEvent.click(saveBtn);
    const alertMsg = await waitForElement(() => getByTestId("alertDescription"));
  ...
  it("should display the storage page with path", async () => {
    ...
    renderComponent();
    const { getByText } = wrapper;
    expect(await waitForElement(() => getByText("Array Management"))).toBeDefined();
