...
class CreateVolume extends Component {
  ...
  componentDidMount() {
    ...
    createVolSocket.on("create_multi_volume", msg => {
      const {total_count} = msg
      const {pass} = msg
      const error_msg = `Status: ${ pass }/${ total_count } volume(s) created successfully`
  ...
  createVolumeInParent() {
    ...
    if (this.state.volume_size.length === 0)
      error_desc = "Please Enter Volume Size"
    else if (this.state.volume_size < 1)
      error_desc = "Volume Size Should be greater than 0"
    else if (this.state.volume_name.length < 1)
      error_desc = "Please Enter Volume Name"
    else if (this.state.volume_count.length === 0)
      error_desc = "Please Enter Volume Count"
    else if (this.state.volume_count < 1)
      error_desc = "Volume Count should be greater than 0"
    else if (this.state.volume_count > parseInt(this.props.maxVolumeCount))
      error_desc = `Volume Count should not exceed ${ this.props.maxVolumeCount}`
    else if (this.state.volume_count > 1 && this.state.volume_suffix < 0)
      error_desc = "Suffix Value cannot be negative"
    else if (this.state.volume_count > 1 && this.state.volume_suffix === null)
      error_desc = "Please Enter Suffix Start Value"
    else if (this.state.maxbw.length === 0)
      error_desc = "Please Enter Maximum Bandwidth (MB/s) "
    else if (this.state.maxiops.length === 0)
      error_desc = "Please Enter Maximum IOPS (KIOPS)"
    else if (this.state.maxbw < 0)
      error_desc = "Max Bandwidth cannot be negative"
    else if (this.state.maxiops < 0)
      error_desc = "Maximum IOPS be negative"
    else if (this.state.maxiops > 0 && this.state.maxiops < 10)
      error_desc = "Invalid value for Maximum IOPS"
    else
      isError = false

    if (isError === true) {
      this.setState({ open: true, alert_description: error_desc });
      return;
    }
  ...
  render() {
    ...
    const max = this.props.maxVolumeCount;
              ...
              >
                <FormControl className={classes.volumeName}>
                  <TextField
                    ...
                    label="Volume Count"
                    type="number"
                    inputProps={{ min: 1, max: this.props.maxVolumeCount }}
