...
export class Performance extends Component {
  ...
  fetchReadBandwidth(t) {
    for (let i = 0; i < this.state.chartContent.length; i++) {
  ...
  fetchWriteBandwidth(t) {
    console.log('Fetching BW', this.state.level)
    for (let i = 0; i < this.state.chartContent.length; i++) {
      if (this.state.level === "array")
  ...
  levelChanged(event) {
    if (event.target.value === 'array') {
      ...
      for (let i = 0; i < this.props.volumes.length; i++) {
          ...
          <main className={classes.content}>
            ...
            <Grid container spacing={3}>
              ...
              {this.state.level === "power" ? (
                this.props.power_sensor_info.map((power,itr) => (
