...
describe('<Storage Management />', () => {
  ...
  it('should create an array', async () => {
    ...
    fireEvent.click(wb);
    fireEvent.click(getByText('/dev/ram0'));
  ...
  it('should create a volume', async () => {
    ...
    const volName = await waitForElement(() => getByLabelText('Volume Name'));
    fireEvent.change(volName, { target: { value: 'vol1' } });
    const volCount = await waitForElement(() => getByLabelText('Volume Count'));
    ...
    const volSize = await waitForElement(() => getByLabelText('Volume Size'));
  ...
  it('should throw error if creating volume is not possible because of missing volume name', async () => {
    ...
    const { getByTestId, getByLabelText, getByText } = wrapper;
    const volCount = await waitForElement(() => getByLabelText('Volume Count'));
    fireEvent.change(volName, { target: { value: 1 } });
    const volName = await waitForElement(() => getByLabelText('Volume Name'));
  ...
  it('should show attach a disk', async () => {
    mock.onGet('/api/v1.0/get_devices/').reply(200, {
      devices,
      "metadevices": ["uram0"]
    })
    ...
    renderComponent();
    const { getByTestId, asFragment, getByText } = wrapper;
    fireEvent.click(await waitForElement(() => getByTestId('attachdisk-5')));
    fireEvent.click(await waitForElement(() => getByText("Yes")));
  });

  it('should show detach a disk', async () => {
    mock.onGet('/api/v1.0/get_devices/').reply(200, {
      devices,
      "metadevices": ["uram0"]
    })
    ...
    renderComponent();
    const { getByTestId, asFragment, getByText } = wrapper;
    fireEvent.click(await waitForElement(() => getByTestId('detachdisk-0')));
    fireEvent.click(await waitForElement(() => getByText("Yes")));
  });
