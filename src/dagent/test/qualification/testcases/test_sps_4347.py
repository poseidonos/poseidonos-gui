...
array_raid = testcase_config["Functional"]["SPS_4347"]["raid_type"]
data_devices = testcase_config["Functional"]["SPS_4347"]["data_devices"]



def test_sps_4347(setup):
    ...
    assert setup.create_array_second(raid_type=array_raid,data_device=data_devices) == True
    assert setup.array_mount_second(write_through_mode=True)
    ...
    assert setup.array_unmount_first() == True
    assert setup.array_mount_first(write_through_mode=True) == True
    assert setup.array_unmount_second() == True
    assert setup.array_mount_second(write_through_mode=False) == True