...
class TargetUtils:
    ...
    def dev_bdf_map(self) -> (bool, dict()):
        ...
        try:
            dev_bdf_map = {}
            self.request.list_devices()
            list_dev_out = self.request.device_map
            if len(list_dev_out) == 0:
                raise Exception("No Devices found")
            for dev in list_dev_out:
                dev_bdf_map[dev] = list_dev_out[dev]["addr"]
    ...
    def get_devs(self, prop={"type": "SSD"}) -> (bool, dict()):
        ...
        """
        self.request.list_devices()
        dev_map = self.request.device_map
